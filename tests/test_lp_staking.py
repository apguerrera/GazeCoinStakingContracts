...
def test_lp_staking(staking_rewards, lp_token):
    lp_token_staker = accounts[5]
    staking_amount = 50 * TENPOW18
    lp_token.approve(staking_rewards,staking_amount,{"from":lp_token_staker})
    before_stake_lp_balance = staking_rewards.getStakedBalance(lp_token_staker)
    
    staking_rewards.zapEth({"from":lp_token_staker,  'value': 1 * 10**18})
    after_stake_lp_balance = staking_rewards.getStakedBalance(lp_token_staker)
